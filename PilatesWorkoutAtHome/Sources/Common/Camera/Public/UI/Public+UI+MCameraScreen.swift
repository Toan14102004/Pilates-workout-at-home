...
///
/// - important: A view conforming to **MCameraScreen** has to be passed directly to ``MCamera``. See
...
public extension MCameraScreen {
    ...
    @MainActor func captureOutput() { cameraManager.captureOutput() }

    /// Set the output type of the camera.
    ///
    /// For available options, please refer to the ``CameraOutputType`` documentation.
    @MainActor func setOutputType(_ outputType: CameraOutputType) { cameraManager.setOutputType(outputType) }
    ...
    @MainActor func setZoomFactor(_ zoomFactor: CGFloat) throws { try cameraManager.setCameraZoomFactor(zoomFactor) }
    ...
    @MainActor func setFlashMode(_ flashMode: CameraFlashMode) { cameraManager.setFlashMode(flashMode) }
    ...
    func setCaptureDelay(_ captureDelay: CaptureDelay) {
        cameraManager.setCaptureDelay(captureDelay)
    }

    /// Set the image aspect ratio for the camera.
    ///
    /// For available options, please refer to the ``ImageAspectRatio`` documentation.
    @MainActor func setImageAspectRatio(_ imageAspectRatio: ImageAspectRatio) {
        cameraManager.setImageAspectRatio(imageAspectRatio)
    }

    /// Set the sound mode for the camera.
    ///
    /// For available options, please refer to the ``Sound`` documentation.
    @MainActor func setSound(_ sound: Sound) {
        cameraManager.setSound(sound)
    }

    /// Set the focus mode for the camera.
    ///
    /// For available options, please refer to the ``Focus`` documentation.
    @MainActor func setFocus(_ focus: Focus) {
        cameraManager.setFocus(focus)
    }

    /// Set the mirror mode for the camera.
    ///
    /// For available options, please refer to the ``Mirror`` documentation.
    @MainActor func setMirror(_ mirror: MirrorType) {
        cameraManager.setMirror(mirror)
    }

    /// Set the vibrate mode for the camera.
    ///
    /// For available options, please refer to the ``Vibrate`` documentation.
    @MainActor func setVibrate(_ vibrate: Vibrate) {
        cameraManager.setVibrate(vibrate)
    }

    /// Set the grid type for the camera.
    ///
    /// For available options, please refer to the ``Grid`` documentation.
    @MainActor func setGrid(_ grid: Grid) {
        cameraManager.setGrid(grid)
    }
}
...
public extension MCameraScreen {
    ...
    var imageAspectRatio: ImageAspectRatio { cameraManager.attributes.imageAspectRatio }
    var captureDelay: CaptureDelay { cameraManager.attributes.captureDelay }
    var sound: Sound { cameraManager.attributes.sound }
    var focus: Focus { cameraManager.attributes.focus }
    var mirror: MirrorType { cameraManager.attributes.mirror }
    var vibrate: Vibrate { cameraManager.attributes.vibrate }
    var grid: Grid { cameraManager.attributes.grid }
    
    var loading: Bool { cameraManager.attributes.loading }
}
