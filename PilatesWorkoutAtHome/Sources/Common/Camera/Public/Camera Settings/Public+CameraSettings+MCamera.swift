...
public extension MCamera {
    ...
    ///
    /// - tip: To hide selected buttons and controls on the screen, use the method with DefaultCameraScreen as argument.
    /// For a code example, please refer to Usage -> Default Camera Screen Customization section.
    ///
    ///
    /// # Usage
    ///
    ...
    ///
    /// For more details and tips on creating your own **Captured Media Screen**, see the ``MCapturedMediaScreen``
    /// documentation.
    ///
    ...
    /// }
    /// ```
    func setCapturedMediaScreen(_ builder: CapturedMediaScreenBuilder?) -> Self { config
        .capturedMediaScreen = builder; return self
    }
...
public extension MCamera {
    ...
    func setCameraOutputType(_ cameraOutputType: CameraOutputType) -> Self { manager.attributes
        .outputType = cameraOutputType; return self
    }
    ...
    func setCameraPosition(_ cameraPosition: CameraPosition) -> Self { manager.attributes
        .cameraPosition = cameraPosition; return self
    }
    ...
    func setAudioAvailability(_ isAvailable: Bool) -> Self { manager.attributes
        .isAudioSourceAvailable = isAvailable; return self
    }
    ...
    func setResolution(_ resolution: AVCaptureSession.Preset) -> Self { manager.attributes
        .resolution = resolution; return self
    }

    /// Changes the initial camera frame rate.
    ///
    /// - note: Depending on the resolution of the camera and the current specifications of the device, there are some
    /// restrictions on the frame rate that can be set.
    /// If you set a frame rate that exceeds the camera's capabilities, the library will automatically set the closest
    /// possible value and show you which value has been set (``MCameraScreen/frameRate``).
    ...
    func setCameraExposureDuration(_ duration: CMTime) -> Self { manager.attributes.cameraExposure
        .duration = duration; return self
    }
    ...
    func setCameraTargetBias(_ targetBias: Float) -> Self { manager.attributes.cameraExposure
        .targetBias = targetBias; return self
    }
    ...
    func setCameraExposureMode(_ exposureMode: AVCaptureDevice.ExposureMode) -> Self { manager.attributes.cameraExposure
        .mode = exposureMode; return self
    }
    ...
    func setGridVisibility(_ shouldShowGrid: Bool) -> Self { manager.attributes
        .isGridVisible = shouldShowGrid; return self
    }
    ...
    func setFocusImageColor(_ color: UIColor) -> Self { manager.cameraMetalView.focusIndicator
        .tintColor = color; return self
    }
...
public extension MCamera {
    ...
    /// }
    /// ```
    func setCloseMCameraAction(_ action: @escaping () -> Void) -> Self { config
        .closeMCameraAction = action; return self
    }

    /// Defines action that is called when an image is captured.
    ///
    /// MCameraController can be used to perform additional actions related to MCamera, such as closing MCamera or
    /// returning to the camera screen.
    /// See ``Controller`` for more information.
    ///
    /// - note: The action is called immediately if **Captured Media Screen** is nil, otherwise after the user accepts
    /// the photo.
    ///
    ///
    /// ## Usage
    ...
    /// }
    /// ```
    func onImageCaptured(_ action: @escaping (UIImage, MCamera.Controller) -> Void) -> Self { config
        .imageCapturedAction = action; return self
    }
    ...
    /// }
    /// ```
    func onVideoCaptured(_ action: @escaping (URL, MCamera.Controller) -> Void) -> Self { config
        .videoCapturedAction = action; return self
    }
}
...
public extension MCamera {
    ...
    /// }
    /// ```
    func lockCameraInPortraitOrientation(_ appDelegate: MApplicationDelegate.Type) -> Self { config
        .appDelegate = appDelegate; manager.attributes.orientationLocked = true; return self
    }
