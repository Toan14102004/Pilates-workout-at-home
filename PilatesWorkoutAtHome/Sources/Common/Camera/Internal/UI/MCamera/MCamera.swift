...
///
/// By default, it includes three screens that change depending on the status of the camera; **Error Screen**, **Camera
/// Screen** and **Captured Media Screen**.
///
/// Handles issues related to asking for permissions, and if permissions are not granted, it displays the **Error
/// Screen**.
///
...
/// - important: To start a camera session, simply call the ``startSession()`` method. For more details, see the
/// **Usage** section.
///
/// ## Camera Screens
...
///   - ``onImageCaptured(_:)``
///   - ``onVideoCaptured(_:)``
/// - note: If there is no **Captured Media Screen**, the action is called immediately after the media is captured,
/// otherwise it is triggered after the user accepts the captured media in the **Captured Media Screen**.
///
...
///   - ``setFocusImageSize(_:)``
/// - important: Note that if you try to set a value that exceeds the camera's capabilities, the camera will
/// automatically set the closest possible value and show you which value has been set.
///
/// ## Other
